...
__global__ void PSROIPoolForwardKernel(
  ...
  CUDA_KERNEL_LOOP(index, count) {
	...
	DType roi_start_z = static_cast<DType>(round(offset_bottom_rois[1])) * spatial_scale;
    DType roi_start_w = static_cast<DType>(round(offset_bottom_rois[2])) * spatial_scale;
    DType roi_start_h = static_cast<DType>(round(offset_bottom_rois[3])) * spatial_scale;
	DType roi_end_z = static_cast<DType>(round(offset_bottom_rois[4]) + 1.) * spatial_scale;
    DType roi_end_w = static_cast<DType>(round(offset_bottom_rois[5]) + 1.) * spatial_scale;
    DType roi_end_h = static_cast<DType>(round(offset_bottom_rois[6]) + 1.) * spatial_scale;

    // Force too small ROIs to be 1x1
	DType roi_depth = max(roi_end_z - roi_start_z,0.1);//avoid 0
	...
	int zstart = floor(static_cast<DType>(pz) * bin_size_z
                        + roi_start_z);
    int hstart = floor(static_cast<DType>(ph) * bin_size_h
                        + roi_start_h);
    int wstart = floor(static_cast<DType>(pw)* bin_size_w
                        + roi_start_w);
	int zend = ceil(static_cast<DType>(pz + 1) * bin_size_z
                      + roi_start_z);
	...
	zstart = min(max(zstart, 0), depth);
	zend = min(max(zend, 0),zend);
	wstart = min(max(wstart, 0),width);
    hstart = min(max(hstart, 0), height);
    hend = min(max(hend, 0), height);	
    wstart = min(max(wstart, 0),width);
    wend = min(max(wend, 0), width);
    bool is_empty = (hend <= hstart) || (wend <= wstart) || (zend <=  zstart);
	
	int gz = floor(static_cast<DType>(pz)* group_size / pooled_depth);	
    int gw = floor(static_cast<DType>(pw)* group_size / pooled_width);
    int gh = floor(static_cast<DType>(ph)* group_size / pooled_height);
	gz = min(max(gz, 0), group_size - 1);
    gw = min(max(gw, 0), group_size - 1);
    gh = min(max(gh, 0), group_size - 1);
    int c = ((ctop*group_size + gz)*group_size + gh) + gw;

    const DType* offset_bottom_data = bottom_data + (roi_batch_ind * channels + c) * height * width*depth;
    DType out_sum = 0;
	for(int z = zstart;z < zend; ++z){
    	for (int h = hstart; h < hend; ++h){
      		for (int w = wstart; w < wend; ++w){
        		int bottom_index = (z * height + h )*width + w;
